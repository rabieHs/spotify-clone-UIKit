...
class PlayListViewController: UIViewController {
    ...
    private let collectionView = UICollectionView(frame:
            .zero,
            ...
            DispatchQueue.main.async {
                ...
                case .success(let model):
                    ...
                    self?.viewModels =  model.tracks.items.compactMap({
                        RecommendedCellViewModel(
                            name: $0.track.name, artistName: $0.track.artists.first?.name ?? "-", artworkURL: URL(string: $0.track.album?.images.first?.url ?? ""))
                    })
    ...
    @objc func didLongPress(_ gesture : UILongPressGestureRecognizer){
        ...
        let touchPint = gesture.location(in: collectionView)
        guard let indexPath = collectionView.indexPathForItem(at: touchPint) else {
            return
        }
    ...
    private func  showSuccessAlert(){
        let alert = UIAlertController(title: "Success", message: "Track successfully Deleted from Playlist", preferredStyle: .alert)
